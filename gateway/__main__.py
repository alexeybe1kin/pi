...
def create_certificate(directory: Path, hostname: str) -> tuple[Path, Path]:
    ...
    if certificate.exists() or key_path.exists():
        if not certificate.is_file() or not key_path.is_file():
            raise ValueError("TLS material is incomplete. Restore both tls.crt and tls.key before starting.")
    ...
    certificate_value = (x509.CertificateBuilder().subject_name(name).issuer_name(name)
...
def main() -> None:
    ...
    if args.command == "serve":
        ...
        if args.command == "revoke-all":
            auth.revoke()
            print("Every browser session was revoked. Already dispatched actions cannot be recalled.")
            return
        print("This password protects browser access to your conversations and approval decisions.\n"
              "Only a salted verifier is kept in this machine's gateway volume; the password is not saved.\n"
              "Anyone controlling this host can reset it. Resetting cannot restore lost data, vault keys\n"
        ...
        print("Password saved. Every previous browser session was revoked. Sign in with the new password.")
