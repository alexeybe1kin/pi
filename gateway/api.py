...
class Config:
    ...
    def validate(self) -> None:
        ...
        if (origin.scheme != "https" or not origin.hostname or origin.username or origin.password
            ...
            raise ValueError("Set GATEWAY_ORIGIN to the exact HTTPS browser origin, with no trailing slash.")
        if len(self.pi_key) < 32 or self.owner_key == self.pi_key:
            raise ValueError("Provision a distinct PI_GATEWAY_KEY of at least 32 characters on the host.")
        for value in (self.pi_url, self.toolgate_url):
            url = urlsplit(value)
            if (url.scheme not in {"http", "https"} or not url.hostname or url.username
                    or url.password or url.path not in {"", "/"} or url.query or url.fragment):
                raise ValueError("Set gateway service URLs to fixed origins without credentials or paths.")

    @classmethod
    def environment(cls) -> Config:
        return cls(os.environ.get("GATEWAY_ORIGIN", ""),
...
def create_app(config: Config | None = None, *, store: AuthStore | None = None,
               transport: httpx.BaseTransport | None = None) -> FastAPI:
    @asynccontextmanager
    async def lifespan(app: FastAPI):
        ...
        with httpx.Client(transport=transport, timeout=660, follow_redirects=False,
                          trust_env=False) as client:
            app.state.client = client
            yield

    app = FastAPI(title="Conker browser gateway", version="0.1.0", lifespan=lifespan,
    ...
    async def boundary(request: Request, call_next):
        ...
        if request.url.scheme != "https" or request.headers.get("host") != urlsplit(settings.origin).netloc:
            return JSONResponse({"detail": "Use the configured HTTPS address to open Conker."}, status_code=400)
    ...
    def auth_session(request: Request):
        ...
        response = JSONResponse({"authenticated": bool(value["authenticated"]), "csrf_token": value["csrf"],
                                 "session_id": value["id"], "expires_at": value["expires"],
    ...
    async def login(request: Request):
        ...
        value = await run_in_threadpool(app.state.auth.login, request.cookies.get(COOKIE, ""),
                                       body["password"], request.client.host if request.client else "unknown")
        return cookie(JSONResponse({"authenticated": True, "csrf_token": value["csrf"],
                                    "session_id": value["id"], "expires_at": value["expires"]}), value["token"])
    ...
    def forward(method: str, url: str, key_header: str, key: str, body: dict | None, query: bytes):
        ...
        if 300 <= response.status_code < 400:
            raise AuthError("Service returned an unexpected redirect. Check gateway service URLs on the host.", 502)
        try:
            data = response.json()
        except ValueError:
            raise AuthError("Service returned an unreadable response. Check its logs before retrying.", 502) from None
        # Upstream headers (especially cookies) never cross the browser boundary.
        return JSONResponse(data, status_code=response.status_code)

    @app.api_route("/api/pi/{path:path}", methods=["GET", "POST"])
    async def runtime(path: str, request: Request):
        ...
        from starlette.concurrency import run_in_threadpool
        return await run_in_threadpool(forward, request.method, app.state.config.pi_url.rstrip("/") + target,
                                      "X-Pi-Gateway-Key", app.state.config.pi_key, body, request.scope["query_string"])

    @app.get("/api/owner/requests")
    def owner_requests(request: Request):
        session(request)
        return forward("GET", app.state.config.toolgate_url.rstrip("/") + "/v2/owner/requests",
                       "X-ToolGate-Owner-Key", app.state.config.owner_key, None, b"")
    ...
    async def owner_decision(identity: str, request: Request):
        ...
        body = await json_body(request)
        if (set(body) - {"status", "note"} or body.get("status") not in {"approved", "rejected", "dismissed"}
                or not isinstance(body.get("note", ""), str)):
            raise AuthError("Send a decision status and optional note.", 422)
        from starlette.concurrency import run_in_threadpool
        return await run_in_threadpool(forward, "POST",
