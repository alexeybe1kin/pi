...
from .providers import Completion, Message, Provider, ProviderUnavailable
...
class Loop:
    def __init__(self, store: Store, provider: Provider, *, model: str,
                 system_prompt: str = "", fork_threshold_chars: int = DEFAULT_FORK_THRESHOLD_CHARS) -> None:
        self.store = store
    ...
    def _summarise(self, messages: list[Message]) -> str:
        ...
        try:
            return self.provider.complete(ask, model=self.model).text.strip()
        except ProviderUnavailable as exc:
            return (f"[summary unavailable: {exc.reason}] The parent session holds the full "
    ...
    def run_turn(self, session_id: str, user_text: str) -> dict:
        ...
        try:
            completion: Completion = self.provider.complete(self._history(session_id), model=self.model)
        except ProviderUnavailable as exc:
            ...
            self.store.finish_turn(turn_id, "failed", provider=self.provider.name,
                                   model=self.model, detail=exc.reason,
                                   latency_ms=int((time.monotonic() - started) * 1000))
            raise TurnFailed(exc.reason) from exc
