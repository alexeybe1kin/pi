...
def catalogue(monkeypatch, pricing, *, paid=False):
    monkeypatch.setattr(httpx, "get", lambda *a, **kw: httpx.Response(200,
        request=httpx.Request("GET", "https://catalogue"), json={"data": [{
            "id": "candidate", "architecture": {"input_modalities": ["text"],
            "output_modalities": ["text"]}, "pricing": pricing}]}))
    return OpenRouterProvider("test", allow_paid=paid)


@pytest.mark.parametrize("pricing", [{}, {"prompt": "0", "completion": "0"},
...
@pytest.mark.parametrize("usage,expected", [({"cost": 0.15}, 0.15),
    ({"cost": 0}, 0), ({}, None), ({"cost": None}, None), ({"cost": ""}, None),
    ({"cost": "nan"}, None), ({"cost": -1}, None)])
def test_actual_cost_is_preserved_and_unknown_never_becomes_zero(monkeypatch, usage, expected):
    provider = catalogue(monkeypatch, {"prompt": "0", "completion": "0", "request": "0.15"}, paid=True)
    monkeypatch.setattr(httpx, "post", lambda *a, **kw: httpx.Response(200,
        request=httpx.Request("POST", "https://chat"), json={
...
def test_fully_zero_catalogue_is_still_callable(monkeypatch):
    provider = catalogue(monkeypatch, {"prompt": "0", "completion": "0", "request": "0", "image": "0"})
...
def test_catalogue_outage_keeps_local_turn_and_records_gap(tmp_path):
    ...
    loop = Loop(store, Router(local_provider=Scripted(["local answer"]),
        local_model="local", hosted_provider=Offline()))
