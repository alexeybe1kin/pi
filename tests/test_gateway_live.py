...
def test_real_https_login_conversation_logout_and_host_recovery(tmp_path, monkeypatch, capsys):
    ...
    worker_env = {**os.environ, "PI_ADMIN_KEY": "host-recovery-key-" + "a" * 32,
                  "PI_GATEWAY_KEY_SHA256": hashlib.sha256(key.encode()).hexdigest(),
                  "PI_DB_PATH": str(tmp_path / "pi.db"), "PI_TOOLGATE_KEY": "",
                  "PI_MEMORYGATE_URL": "", "PI_MEMORYGATE_INGEST_KEY": "", "PI_MEMORYGATE_READ_KEY": "",
                  "PI_OPENROUTER_KEY": ""}
    gateway_env = {**os.environ, "GATEWAY_ORIGIN": origin, "GATEWAY_DB_PATH": str(auth.path),
                   "GATEWAY_PI_URL": f"http://127.0.0.1:{pi_port}", "PI_GATEWAY_KEY": key,
                   "GATEWAY_TOOLGATE_OWNER_KEY": ""}
    processes = []
    with (tmp_path / "process.log").open("w") as log:
        try:
            for args, env in ((["uvicorn", "pi.api:app", "--host", "127.0.0.1", "--port", str(pi_port)], worker_env),
            ...
            with httpx.Client(base_url=origin, verify=ssl.create_default_context(cafile=str(cert)), trust_env=False) as client:
                ...
                while True:
                    try:
                        ...
                        assert time.monotonic() < deadline, "Gateway or Pi failed to start; inspect process.log"
                        time.sleep(0.1)
                ...
                saved = client.post("/api/pi/sessions", json={"title": "Before school / До школы"}, headers=headers)
                assert saved.status_code == 200, saved.text
                assert client.get("/api/pi/sessions").json()["results"][0]["id"] == saved.json()["session_id"]
                old_cookie = client.cookies.get(COOKIE)
                assert client.post("/auth/logout", headers=headers).status_code == 200
                assert client.get("/api/pi/sessions", headers={"Cookie": f"{COOKIE}={old_cookie}"}).status_code == 401
                from gateway import __main__ as cli
                # Supply terminal input without opening an interactive Windows console.
                monkeypatch.setattr(cli.getpass, "getpass", lambda prompt: "new passphrase after recovery")
                monkeypatch.setattr(sys, "argv", ["gateway", "reset-password", "--db", str(auth.path)])
                cli.main()
                ...
                result = client.post("/auth/login", json={"password": "new passphrase after recovery"},
                                     headers={"Origin": origin, "X-CSRF-Token": csrf})
                assert result.status_code == 200
                assert client.get("/api/pi/sessions").json()["results"][0]["title"] == "Before school / До школы"
        finally:
